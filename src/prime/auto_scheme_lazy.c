...
return_value = autoscheme_eval(cons(mk_symbol("define-library"),cons(cons(mk_symbol("auto"),cons(mk_symbol("scheme"),cons(mk_symbol("lazy"),NIL))),cons(cons(mk_symbol("export"),cons(mk_symbol("delay"),cons(mk_symbol("force"),NIL))),cons(cons(mk_symbol("begin"),cons(cons(mk_syntax(OP_DEF0,"define"),cons(mk_symbol("delay"),cons(mk_proc(OP_DELAY,&NIL),NIL))),cons(cons(mk_syntax(OP_DEF0,"define"),cons(mk_symbol("force"),cons(mk_proc(OP_FORCE,&NIL),NIL))),NIL))),NIL)))), environment);
return return_value;
}
