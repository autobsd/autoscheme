...
return_value = autoscheme_eval(cons(mk_symbol("define-library"),cons(cons(mk_symbol("auto"),cons(mk_symbol("scheme"),cons(mk_symbol("macro"),NIL))),cons(cons(mk_symbol("export"),cons(mk_symbol("define-macro"),NIL)),cons(cons(mk_symbol("begin"),cons(cons(mk_syntax(OP_DEF0,"define"),cons(mk_symbol("define-macro"),cons(mk_syntax(OP_DEFMACRO0,"define-macro"),NIL))),NIL)),NIL)))), environment);
return return_value;
}
